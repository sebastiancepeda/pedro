...
def segment_plates(params):
    ...
    min_area = 14 ** 2
    max_area = 40 ** 2
    contours = [get_contours_gray(im, min_area, max_area) for im in images]
    print("Draw contours")
    images = [cv2.drawContours(
        cv2.cvtColor(im, cv2.COLOR_GRAY2RGB), c, -1, color, thickness, 8
    ) for im, c in zip(images, contours)]
    print_named_images(images, output_folder, "mask_contours")
