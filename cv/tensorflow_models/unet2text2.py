...
def get_model_definition(img_height, img_width, in_channels, out_channels):
    ...
    h_dim = 20
    # Down
    c1 = Conv2D(h_dim, **kwargs_conv2d)(pre_processing)
    c1 = Conv2D(h_dim, **kwargs_conv2d)(c1)
    c2 = MaxPooling2D((2, 2))(c1)
    c2 = Conv2D(h_dim, **kwargs_conv2d)(c2)
    c2 = Conv2D(h_dim, **kwargs_conv2d)(c2)
    c3 = MaxPooling2D((2, 2))(c2)
    c3 = Conv2D(h_dim, **kwargs_conv2d)(c3)
    c3 = Conv2D(h_dim, **kwargs_conv2d)(c3)
    c4 = MaxPooling2D((2, 2))(c3)
    c4 = Conv2D(h_dim, **kwargs_conv2d)(c4)
    c4 = Conv2D(h_dim, **kwargs_conv2d)(c4)
    c5 = MaxPooling2D(pool_size=(2, 2))(c4)
    c5 = Conv2D(h_dim, **kwargs_conv2d)(c5)
    c5 = Conv2D(h_dim, **kwargs_conv2d)(c5)
    # Up
    u4 = Conv2DTranspose(h_dim, (2, 2), strides=(2, 2), padding='same')(c5)
    u4 = concatenate([u4, c4])
    u4 = Conv2D(h_dim, **kwargs_conv2d)(u4)
    u4 = Conv2D(h_dim, **kwargs_conv2d)(u4)
    u3 = Conv2DTranspose(h_dim, (2, 2), strides=(2, 2), padding='same')(u4)
    u3 = concatenate([u3, c3])
    u3 = Conv2D(h_dim, **kwargs_conv2d)(u3)
    u3 = Conv2D(h_dim, **kwargs_conv2d)(u3)
    u2 = Conv2DTranspose(h_dim, (2, 2), strides=(2, 2), padding='same')(u3)
    u2 = concatenate([u2, c2])
    u2 = Conv2D(h_dim, **kwargs_conv2d)(u2)
    u2 = Conv2D(h_dim, **kwargs_conv2d)(u2)
    u1 = Conv2DTranspose(h_dim, (2, 2), strides=(2, 2), padding='same')(u2)
    u1 = concatenate([u1, c1], axis=3)
    u1 = Conv2D(h_dim, **kwargs_conv2d)(u1)
    u1 = Conv2D(h_dim, **kwargs_conv2d)(u1)
    # Downward
    kwargs_conv2d = {
        'activation': 'relu',
